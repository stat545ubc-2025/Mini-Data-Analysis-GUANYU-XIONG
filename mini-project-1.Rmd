...
```
This chunk explores *apt_buildings*, showing the number of rows, number of columns, dataset class type, and a glimpse of variable names and types.
```{r}
### building_permits ###
print(
  building_permits %>%
...
```
This chunk explores *building_permits*, giving the number of rows, columns, dataset class type, and variable structure.
```{r}
### cancer_sample ###
print(
cancer_sample %>%
...
```
This chunk explores *cancer_sample*, reporting its rows, columns, class type, and variable information.
```{r}
### flow_sample ###
print(
  flow_sample %>%
...
```
This chunk explores *flow_sample*, showing the number of rows, number of columns, dataset class type, and variable types.
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
I will choose apt_buildings cause that seems most understandable to me comparing to others like cancer_samples. So I intuitively can explain and learn from my analysis result.
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
Whats the relationship between number of unites and other variables like AC. Intuitively that can tell me larger if apartments are tend to have better facility or smaller are.
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
I would choose number *1 numeric variable distribution*. My data include built year this numeric row do make sense because that reflect a general view of how old those apartments are.
```{r}
library(ggplot2)
...
```
Secondly, I would choose *3missing value*. This is easy to explain and I see a NA in last row. Those information could help me on data preprocess.
```{r}
library(dplyr)
library(ggplot2)
library(tidyr)
...
Thirdly, I would choose *4 relationship between 2 variables* because this helps me to see if older apartment have more room or smaller have.
```{r}
library(ggplot2)
...
Finally, I would choose *7 Make a new tibble subset* because this also provide a clearer view of the data im working with.Ex.The subset I'm making now exclude all row that has more than 500 missing sample.
```{r}
library(dplyr)
library(tidyr)
